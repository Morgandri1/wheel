...
SDK/multiplayer lane, 2026-09-11. Companion: `docs/proposals/shared-projects.md` (M1), which
consumes the principal this proposal defines.

**Status, 2026-09-18.** M1 has since landed on `dev` through its own PRs — shared projects, the
admin/prompter/guest tiers, membership, the actor headers, `on_behalf_of`, roster masking, invites
and the members panel are all present, in the form `dev` settled on rather than the form this
lane's branch had. M0 is landing separately, on top of that, tracked in issue #132. Two things in
this document changed on the way, and are marked where they appear: the `external_identities`
migration is **0007**, because 0006 is M1's `project_members`; and §3 now names the neutral
`WHEEL_JWKS_*` variables, with the `CLERK_*` spellings aliased and deprecated rather than retired
(§3.4).
...
| `WHEEL_EXTERNAL_AZP` | no | `azp` allowlist, same semantics as `WHEEL_JWKS_AZP` (§3.4). |
...
issuer's own origin — is the one that must not be used, and the reason is concrete rather than
stylistic. An issuer that serves several of its own surfaces typically mints tokens for all of them
under one issuer, and several of those already carry the issuer origin as their `aud`: AgentGrid's
desktop, mobile and relay tokens do exactly this (issue #132). Configure `WHEEL_EXTERNAL_AUDIENCE`
to the issuer origin and every one of those becomes a valid Wheel cloud credential, which is attack
#3 with the control switched on and pointed the wrong way. So:

```
WHEEL_EXTERNAL_ISSUER=https://accounts.agentgrid.example      # what the IdP puts in `iss`
...
### 3.4 The `CLERK_*` names are aliased and deprecated, not retired

`AUTH_MODE=jwks` reads `CLERK_JWKS_URL`, `CLERK_ISSUER` and `CLERK_AZP`. Nothing in that mode is
Clerk-specific — it is an OIDC issuer and a JWKS — and a variable named after one vendor describes a
configuration this project no longer has. Issue #132 asks for one of two answers. This is the
choice, and the reason:

* **Retire (rename, hard break).** `CLERK_JWKS_URL` and `CLERK_ISSUER` are **required** under `jwks`
  and empty values refuse to boot, so a rename turns a documentation change into a failed deploy on
  the next restart of every deployment configured the old way — including the one in
  `infra/docker-compose.yml` and the Railway service. That is an outage caused by a doc fix.
* **Alias with a deprecation warning.** `WHEEL_JWKS_URL`, `WHEEL_JWKS_ISSUER` and `WHEEL_JWKS_AZP`
  become the documented names and are read first. The `CLERK_*` spellings keep working and log a
  one-line deprecation at boot naming both the old and new variable. Setting **both** to
  *different* values refuses to boot, so the alias can never become an ambiguity about which issuer
  is pinned — the one failure mode an alias could otherwise introduce.

**Alias.** The requirement in #132 is that the *documentation* stop describing a Clerk-era
configuration, and an alias achieves that immediately at zero deployment risk, while a rename buys
the same doc change with an outage. The removal is not open-ended: `docs/API.md` marks the `CLERK_*`
row deprecated, and dropping the alias is its own change, taken deliberately once no deployment
reads them.
...
New in migration **0007**, in both dialects. Not 0006: that number belongs to M1's
`project_members`/`project_invites`, which landed first (§2 of `shared-projects.md`). Verified by
listing the two migration directories rather than by reading a plan that said otherwise — the same
check that caught the brief's stale "0005", which `api_token_sessions` already holds:
...
- **Reused** — the IdP re-assigns a retired `sub` to a different human. The new human **inherits the
  ...
  (c) `last_seen_at` makes a dormant-then-active identity visible to an operator who looks. It is
  named here as a **residual**, for ADVERSARY to track in `redteam/` — `redteam/` is theirs to
  write, so this document names the residual rather than asserting a finding file that does not
  exist yet.
...
1. **Boot refuses** `proxy_header` when `WHEEL_TRUSTED_PROXIES` is empty **or covers every
   ...
   `bits == 0` is the whole predicate, because it is the only prefix length that
...
3. **The subject and email headers never cross the proxy hop** to the host or the engine — the same
   rule that already keeps `x-auth-token` from crossing it. They cannot join `hop::CLIENT_ONLY`,
   which is a `const` list, because their names are *configured*; so `http::actor` takes the
   deployment's configured names alongside the `x-wheel-` namespace it already strips, and every
   outbound path goes through that one function (`actor::sanitized_with_actor`). A proxy-asserted
   identity that reached an engine would let an agent replay it back at the API as its author.
...
| 6 | Issuer confusion between the two JWKS-backed modes | A `jwks`-mode `kid` satisfying an external token, or vice versa | Separate `JwksCache` instances; `iss` pinned per mode; boot refuses `WHEEL_EXTERNAL_ISSUER` equal to `WHEEL_JWKS_ISSUER` (`CLERK_ISSUER`) or to `PUBLIC_BASE_URL` |
...
| 12 | Subject reuse by the IdP | Inherited account | §4.3 — **residual**. Mitigated, not closed. Named for ADVERSARY to track in `redteam/` |
