...
**Author:** SDK/Engine. **Status:** proposed, nothing below is built except §0.
...
goes in git"), the rulings requested are collected in §6.
...
structural, and needs all four of these:
...
2. **Refused at boot unless the listener is local.** `Config::from_env`
   (`crates/wheel-engine/src/config.rs:107`) exits non-zero if Desktop is requested with a `tcp://`
   listener that is not loopback. `ListenAddr` (`crates/wheel-core/src/spawn.rs:75`) already
   distinguishes `unix://` from `tcp://`.
...
4. **Visible from outside.** `EngineInfo.profiles` reports `desktop` only when it is active, so a
   hosted client can verify its absence rather than assume it.
...
- **Envelope opt-out:** the `<AgentPrompt>` envelope is the only thing that attributes a message
  ...
  (`from=user`). Otherwise an agent→agent or endpoint body reaches the child unframed. A ruling is
  requested on that constraint (R4).
...
- **Send to a Stopped agent does not start it.** This is *not* filed as a bug. `deliver`
  (`supervisor/mod.rs:1778-1791`) starts only a `Parked` agent, and that is what the contract says:
  - "a message never starts a process — it is enqueued" (ARCHITECTURE §3c#13);
  - "Stopped agents queue; queue drains on start" (ARCHITECTURE §3, "Message delivery contract");
  - "A message never starts a process" (PROTOCOL §2 "Agents").

  `Stopped` is the operator's explicit off, and `Parked` is the engine's own. **Recommendation:**
  leave the engine as it is. The plan's `execution.ts` already does "ensure node → start → send", and
  `start` is idempotent (§3c#13). Changing this is a contract change that needs a ruling (R3).
...
- **Anchors:**
  - `docs/PROTOCOL.md:220` lists interrupt as M2, and it is not routed (`api/mod.rs`, `router`).
...
- **R1:** approve the Phase 1 order in §2, with item 10 owned by `api/headless-first`.
- **R2:** approve the four-part Desktop gate in §1 as the minimum. In particular, confirm that a
  non-loopback `tcp://` listener with Desktop requested is a boot failure, not a warning.
- **R3:** keep "a message never starts a process" (§3c#13) and have AgentGrid call `start` before
  `send`. The alternative is amending the contract so a send to a `Stopped` agent starts it.
  SDK recommends keeping the contract.
- **R4:** the envelope opt-out (item 4) is valid only for agents that receive nothing but `from=user`
  traffic.
- **R5:** ADVERSARY review is required before merge for items 2, 3 and 8.
- **R6:** add `GET /v1/engine` to the ARCHITECTURE §4 route list (PM-owned), which does not name it
  today.
