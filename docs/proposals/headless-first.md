...
| 4 | Clean shutdown | SIGTERM/SIGINT stops every embedded engine. Each lets turns in flight finish (up to 20 s), marks any that cannot — and any killed out of band while it was waiting — as interrupted without re-running them, then kills every agent's whole process group before `wheeld` exits. Stopping one project does the same for that project while the daemon keeps running. Natively, run under systemd with `KillMode=mixed` (not `control-group`, its default): `mixed` signals `wheeld` alone, so its own drain runs the sequence above instead of racing systemd's direct signal to the agents. A descendant that leaves its agent's process group (`setsid`) is still out of `wheeld`'s own reach; `TimeoutStopSec`'s cgroup-wide SIGKILL, or the Docker image's container PID namespace, is what ends it. |
...
- **A child killed independently of the supervisor's own signal is covered too** (review round 2, finding
  1). `KillMode=control-group` SIGTERMs the agent process at the same instant as `wheeld`, so it can die
  before `Supervisor::shutdown`'s own drain-and-signal sequence ever reaches it; the exit is caught by
  `reap`, not by `shutdown`'s loop. `reap` now checks the same closing flag and, while it is set, consumes
  the in-flight message as interrupted instead of requeuing it — the fix is `KillMode=mixed` (property 4,
  A9) plus this.
- **Residual, accepted, not fixed:** the message-delivery layer never rewrites an interrupted turn's
  bytes to the child's stdin again, but the harness's own session transcript (what `--resume` reloads)
  already has whatever was written before the kill — the child received the bytes even though it never
  produced a `result`. The next turn on that resumed session can still see the interrupted message in its
  own history and act on it, which this design does not and cannot prevent without parsing and rewriting
  the harness's own session file.
...
| Variable | Value on the VPS | Why |
|---|---|---|
| `BIND_ADDR` (or `--bind`) | `127.0.0.1:8080`, the default, for a native wheeld. In the container, `0.0.0.0:8080`, with the port published on `127.0.0.1` only | Only Caddy and wheel-web reach it |
| `WHEEL_DATA_DIR` (or `--data-dir`) | e.g. `/var/lib/wheel`, or `/data` in the image | `0700`. Holds `master.key`, `operator-token`, `wheel.db` and the projects. Back it up as a secret |
| `PUBLIC_BASE_URL` | `https://<domain>` | Ingress URLs and the session issuer |
| `WHEEL_TRUSTED_PROXIES` | `127.0.0.1/32,::1` when Caddy runs on the host and wheeld natively. When wheeld runs in Docker, Caddy's address as the container sees it: the compose network's gateway or Caddy's container address, never a whole public range | The only peer whose `X-Forwarded-For` is believed |
| `WHEEL_ALLOWED_HOSTS` | `<domain>`, plus the compose service name if wheel-web calls wheeld by name | The DNS-rebinding guard |
| `WHEEL_SIGNUP` | `closed` | The owner adds accounts: `POST /v1/auth/users` |
| `CORS_ALLOWED_ORIGINS` | unset | wheel-web calls from its server |
| wheel-web: `WHEEL_API_URL`, `WHEEL_AUTH_MODE` | `http://127.0.0.1:8080` (or `http://wheeld:8080` in compose), `local` | Server-side calls only |

The kit reads the first token with `cat <data-dir>/operator-token`, or with `docker exec … cat
/data/operator-token`. It mints more with `wheeld token create`, or over HTTPS with
`POST /v1/auth/tokens`.
...
| A5 | A holder of a leaked token (shell history, CI log, `ps`, a lost laptop running AgentGrid) | Keep or extend access | 256-bit secret with immediate revocation (checked on every request, no cache). **Lineage revocation**: revoking the leaked token revokes every token minted from it. Minting is limited to 20 per account per hour. `last_used_at` and `minted_by` make use and fan-out visible in `GET /v1/auth/tokens`. A token-only account has no password to change. The README quickstart keeps the token out of argv (`curl -H @<(…)`) | While live, a holder can mint (rate-limited) tokens and revoke the account's other tokens: they hold the account. A token is refused if any ancestor is revoked, so a child minted in the instant before its parent's revocation dies with the family. A password change revokes every token that account's sessions minted. **A leaked owner token can create accounts (`POST /v1/auth/users`) that survive its revocation**: accounts do not yet record which token created them, and there is no owner-side account list (F8). A token placed in argv is visible via `ps` for the life of that process. Tokens do not expire (F4) |
...
| A9 | Orphaned or zombie agent processes (cost and integrity, not an attacker) | Keep running, and keep spending, after stop | Process-group kill on stop, park and shutdown; bounded drains, so a stop always reaches the agents and never replays a turn a cgroup-wide kill ended mid-flight (review round 2, finding 1). `tini` as PID 1 in the image, and the container's PID namespace ends everything with it. Natively, systemd's `KillMode=mixed` signals `wheeld` alone — letting its own drain-then-signal sequence run instead of racing it — and `TimeoutStopSec`'s cgroup-wide SIGKILL remains the backstop that reaches an escaped descendant if `wheeld` itself does not exit in time | **Not unconditional.** Natively, outside systemd, a descendant that double-forks and calls `setsid` leaves its agent's process group and survives SIGTERM (review round 1). So does anything, if `wheeld` itself is SIGKILLed before `TimeoutStopSec`. A Linux child subreaper was considered and deferred: it makes every escaped orphan `wheeld`'s child, and reaping those while tokio reaps its own children races for exit statuses, so it is not the simple fix the ruling allowed. Cgroup per engine is F7. `PR_SET_PDEATHSIG` fires when the spawning *thread* exits, and tokio's threads come and go. `KillMode=control-group` was tried first and rejected (review round 2, finding 1): it SIGTERMs every agent at the same instant as `wheeld`, before the drain even starts, which turned a turn already running into one replayed after restart |
...
| A14 | A hostile page the operator visits, when `/v1` shares the web app's origin behind the proxy | Ride the browser's web-session cookie (`__Host-wheel_session`) into authenticated `/v1` calls (CSRF) | **The API never authenticates by cookie.** Credentials count only in `x-auth-token` or `Authorization: Bearer`, which a cross-site page cannot set without a CORS grant, and there is none. `tests/no_cookie_auth.rs` holds a valid session or token in a cookie to `401` on every route, and a cookie-only logout to a no-op. The VPS kit also strips `Cookie` on `/v1` and `/p` at Caddy, as a second line | None known. A future cookie-auth path would have to delete that test |
| A13 | A public caller forging `X-Forwarded-For` or `x-wheel-client-ip` | Dodge the engine's per-caller ingress limit or `ip_allow`, or blame another address | `X-Forwarded-For` is read only from a peer inside `WHEEL_TRUSTED_PROXIES` (default none), right to left, stopping at the first untrusted hop. The caller's `x-wheel-*` headers are dropped and `x-wheel-client-ip` is set by the API alone. `X-Forwarded-Proto` is never read | An operator who trusts too wide a range (e.g. `0.0.0.0/0`) hands every caller the choice of address. **Trusting loopback trusts every process on the box, agents included** (review round 1): with `WHEEL_TRUSTED_PROXIES=127.0.0.1/32`, a native agent can connect over loopback and name any client address to `/p/<another project>`, dodging that endpoint's `ip_allow` and per-caller limit. A dedicated loopback alias (e.g. `127.0.0.2`) only narrows who does it by accident, since any local process may bind that source address. The boundary is a network the agents are not on: the VPS kit's Docker network, where the trusted address is Caddy's container and a non-root agent in the wheeld container cannot originate from it, or a unix-socket listener only the proxy can open (F9). Natively this adds nothing to A7: agents there already run as wheeld's uid |
...
- **F9 — a unix-socket listener for wheeld's API**, so a native reverse proxy can be the only thing trusted
  for `X-Forwarded-For` (A13).
