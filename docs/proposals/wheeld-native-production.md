...
|---|---|---|
| **`OOMPolicy=continue`** | — | **The single most important line in this lane.** systemd's default is `DefaultOOMPolicy=stop`: when *any* process in a unit's cgroup is OOM-killed, systemd **stops the unit**. Left at the default, one runaway agent hitting `MemoryMax` takes down `wheeld` and every other project's agents with it. `continue` makes systemd ignore the kill; the engine sees its child die and marks the turn interrupted, which is the behaviour the product already has for a killed agent (`INTERRUPTED_BY_SHUTDOWN`, `supervisor/mod.rs:163`). Nobody would think to write this line, and without it every other limit here is a loaded gun pointed at the daemon. Proven in the rehearsal by actually OOMing a child and asserting `wheeld` is still `active (running)` and still serving. **It prevents a second, independent failure too**, raised by the `sdk/auto-update` lane on reviewing this: a unit that goes down because an agent was OOM-killed looks to their update watchdog exactly like a new build failing its health check, so it would roll back a perfectly good binary. This line is load-bearing for that lane as well as this one. |
