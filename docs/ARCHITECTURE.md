...
- **M1.6 — WHEEL ON WHEEL (operator: ASAP, right after M1.5).** A project `wheel-dev` on the Railway deployment whose agents run
  ...
  - SDK: **Vault node** (encrypted at rest with `WHEEL_VAULT_KEY`; `PUT /v1/vault/:id/:key` write-only; agent→vault `read` wire exports the keys
    ...
    agents on a hosted board log in with no UI step. **Multiple accounts per provider (operator requirement):** credentials live in vault nodes, one vault per account (e.g. `anthropic-alice`, `anthropic-team`, `openai-ci`); an agent uses whichever vault it has a `read` wire to. Recognised credential keys: `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`), `ANTHROPIC_API_KEY`, `CODEX_API_KEY`, and later `CODEX_AUTH_JSON` (written to the node's private HOME). An agent wired to two vaults that both define the same credential key is refused at wire time (ambiguous) — never silently resolved. The agent inspector shows which vault the credentials come from. **Dev toolchain in the image**: `git`, `gh`, `rustup` (stable + clippy + rustfmt), `pnpm`,
