...
**Why these rules held (ADVERSARY, 2026-09-07 — the meta-rule above the rules):** the measured-evidence
culture worked because it was *cheap to be wrong out loud and expensive to be wrong silently*. That incentive
was made real by acting fast on retractions rather than penalizing them — every self-caught error tonight
(035 link 5, 041's trigger, the Low-vs-Medium on 046, QA's coverage revert, API's clippy-through-a-pipe, PM's
346-crate number and shared-worktree reset) was surfaced by the person who made it, before anyone else found
it, *because surfacing it cost them nothing and hiding it would have cost the swarm hours*. A rule that
...
### The shared target dir is bounded, because cargo never collects it (PM, 2026-09-07)

`~/.cargo/config.toml` points every worktree at one `target-dir` so six agents compile each dependency once
rather than six times. That is correct and stays. What was missing is that **cargo has no garbage collector**:
debug artifacts accumulate per test binary per worktree and nothing ever removes them. The directory reached
150 GB on a 460 GB disk with 2.5 GB free.

`infra/trim-target.sh` reports the size and exits non-zero above `TARGET_CEILING_GB` (default 20); `--apply`
prunes incremental state and coverage artifacts, which cargo rebuilds on demand. It never touches `release`,
which is what the binary size gate measures.
...
- **ONE WORKTREE PER SESSION (binding; observed collisions ×3 — §3c #13 evidence).** YOKE may run several sessions of one agent at once. Before editing,
  a session checks `git -C /Users/metatron/wheel-wt/<role> status --short`; if the tree is dirty or another session of your agent is live, do NOT touch it —
...
| 3 | Sender can't tell whether what arrived is what was sent. | `wheel msg` returns `{id, sha256, bytes, state}` where `sha256`/`bytes` are of the ORIGINAL body; the engine stores them and the UI shows them. **On the wire, the body is the escaped form of the original, exactly and deterministically** (§3 envelope escaping — the only transformation applied), so `transcript bytes == escape(original)` and `wheel inbox <id>` returns the original. A test proves both directions with a 200 KiB body containing every ASCII punctuation char, unicode, and forged open/close tags, checked against an implementation of the escaping rule independent of wheel-core. | M1 |
...
- **M1.6 — WHEEL ON WHEEL (operator: ASAP, right after M1.5).** A project `wheel-dev` on the Railway deployment whose agents run
  ...
  - SDK: **Vault node** (encrypted at rest with `WHEEL_VAULT_KEY`; `PUT /v1/vault/:id/:key` write-only; agent→vault `read` wire exports the keys
    ...
    agents on a hosted board log in with no UI step. **Multiple accounts per provider (operator requirement):** credentials live in vault nodes, one vault per account (e.g. `anthropic-alice`, `anthropic-team`, `openai-ci`); an agent uses whichever vault it has a `read` wire to. Recognised credential keys: `CLAUDE_CODE_OAUTH_TOKEN` (from `claude setup-token`), `ANTHROPIC_API_KEY`, `CODEX_API_KEY`, and later `CODEX_AUTH_JSON` (written to the node's private HOME). An agent must never have two reachable vaults defining the same credential key; the engine enforces this at wire creation (wire refused), at `PUT /v1/vault/:id/:key` (409 `ambiguous_credential` naming both vaults and the key), and at spawn (the agent refuses to start with that reason) — never silently resolved. Redaction of vault values from board/logs/transcripts protects against accidental echo; it is not a containment boundary (an agent is untrusted code holding the secret). The agent inspector shows which vault the credentials come from. **Dev toolchain in the image**: `git`, `gh`, `rustup` (stable + clippy + rustfmt), `pnpm`,
