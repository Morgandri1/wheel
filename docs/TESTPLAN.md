...
| `E2E-mp-failed-redeem-keeps-session` | A dead (used/expired/revoked/unknown) invite link does not sign the visitor out: the API answers it with 401 and web clears the session cookie on any 401 in local mode, so the visitor's next request is unauthenticated. A stale link must cost the visitor the link, not their login. | S2 |
