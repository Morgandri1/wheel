...
| `ENG-diskfull-says-so` | On a volume with no free space, the engine's **first** line names the disk being full. Measured today it says `unable to open database file: Error code 14`, which sends the reader after a corrupt database or a permission problem. ENOSPC is the one condition sqlite cannot describe and the filesystem can, and the operator spent the first part of an 80-minute outage reading sqlite errors about shared memory. A cause the message does not name is a cause somebody has to guess. | S2 |
