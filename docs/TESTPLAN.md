...
| `AUTH-ext-jwks-honours-use-and-key-ops` | A key the issuer published as `use: "enc"`, with an unreadable `use`, or with a `key_ops` lacking `verify` (including an empty one) is never held; `use: "sig"`, a `key_ops` containing `verify`, and **silence** are all held. The key set was telling us what each key was for and we inferred purpose from key type alone — the same class of mistake as taking the algorithm from the token, since the authority is the document fetched from the issuer. Absent stays unconstrained deliberately: most issuers omit both, and refusing silence breaks nearly every real key set. | S2 |
...
| `AUTH-ext-proxy-cross-site-get` | `Sec-Fetch-Site: cross-site` and `same-site` are refused **on their own**, case-insensitively, and an otherwise-allowed `Origin` does not buy a pass. `none` and `same-origin` pass, and a client sending neither header still passes. A cross-site `GET` carries no `Origin` — Fetch appends it only for non-`GET`/`HEAD` or `cors`/`websocket` — so `<img src>`, `<script src>`, `<iframe src>` and a link click were all page-caused authenticated requests walking through a check built to stop exactly that. | S2 |
...
| `AUTH-ext-cfg-audience-shadows-issuer` | `ExternalAuth::audience_shadowing_the_issuer` identifies a configured audience equal to the pinned issuer or its origin. `cross_check` **refuses to boot** on it unless `WHEEL_EXTERNAL_ALLOW_ISSUER_AUDIENCE=1` (test `an_audience_that_is_the_issuers_origin_refuses_to_boot_unless_overridden`), and the detection is a pure function precisely so this rule can be asserted: a control that lives only inside a `tracing::warn!` cannot be tested, and an untestable control silently stops firing. | S2 |
