...
| `ENG-escaper-never-panics` | The envelope escaper never panics, with the boundary landing at **every offset** over an em dash (3 bytes), an emoji (4), CJK (3) and a ZWJ family sequence. A fixed body exercises one alignment; the panic shape is a boundary landing mid-character, so the character is walked across the boundary by padding 0..7 bytes. | **S1** |
