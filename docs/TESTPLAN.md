...
| `ENG-park-resume-passes-session-to-child` | The child is spawned with `--resume <the pre-park session id>`, read from the harness's recorded argv. **This is the assertion that matters, and a matching `session_id` column is not it**: the engine could store the id and start a fresh context, and `/v1/board` would look identical. This checks the mechanism by which context survives, not the bookkeeping around it. | **S2** |
| `ENG-park-zero-never-parks` | `idle_timeout_secs: 0` means never park — an agent that must stay hot. | S3 |
...
| ID | Criterion | Sev |
|---|---|---|
| `E2E-mp` | Invites and tiers against the REAL `wheel-api` (sqlite, local auth) and the packaged board; only the sandbox host is stubbed (`qa/e2e/support/stub-host.mjs`, which records every engine request). `web/mock` has no tiers, members or invites, so a mock-backed test here would test the mock. Run by `make test-multiplayer` / the `multiplayer` CI job. Group of the rows below. | S1 |
| `E2E-mp-invite-redeem` | An admin creates a guest invite in the Members dialog; a second account opens `/app/invite/<token>`, lands on `/app/<project>`, and `GET /v1/projects/<id>` reports `tier: guest` — the tier the invite named. | **S1** |
| `E2E-mp-invite-single-use` | The same link redeemed by a third account shows the invite error and grants nothing (the project stays a 404 for them); the answer never says whether the link is unknown, used, expired or revoked. | **S1** |
| `E2E-mp-failed-redeem-keeps-session` | A dead (used/expired/revoked/unknown) invite link does not sign the visitor out: the API answers it with 401 and web clears the session cookie on any 401 in local mode, so the visitor's next request is unauthenticated. A stale link must cost the visitor the link, not their login. **Known bug — the spec is annotated `test.fail` until web fixes it.** | S2 |
| `E2E-mp-failed-redeem-grants-nothing` | After a dead invite link the visitor (signed in again) is still a non-member: the project is a 404 for them. | **S1** |
| `E2E-mp-prompter-tier` | A prompter invite lands the redeemer at `prompter`, not `guest`. | S1 |
| `E2E-mp-guest-ui` | A guest sees the roster and the creator row and none of the admin controls (no create-invite, no add-member). | S2 |
| `E2E-mp-guest-denied` | A guest gets 403 on every admin route — invites (create/list), members, project patch/delete/stop, `builder/turns`, engine node create, agent send — and NONE of the refused engine calls reaches the host; a guest's board read does (200, one recorded request). Includes the `builder/turns` route (finding 064). | **S1** |
| `E2E-mp-prompter-bounds` | A prompter is refused invites, `builder/turns`, node create and project delete (403, host never contacted) but may message an agent (200, host contacted once). | **S1** |
| `E2E-mp-nonmember` | An account with no membership gets 404 (not 403) on the project, its members, invites and engine board — it cannot tell the project exists. | **S1** |
| `E2E-mp-admin-still-admin` | The creator keeps `tier: admin` and can still list invites after others have joined. | S2 |
...
| `QA-image-fresh` | The container image under test must not predate the code under test. `pin_image()` compares the image's build time against the last commit touching `crates/` or `docker/` and RAISES `StaleImage`; `run_suite` turns that into a SKIP with the reason, never a pass. On 2026-09-06 `wheel-engine:test` was six hours old and every suite run against it that afternoon reported green against a pre-fix engine — including a deploy decision. Override deliberately with `WHEEL_ALLOW_STALE_IMAGE=1`. The comparison is a pure function (`staleness`) so it can be shown to report stale without waiting six hours for an image to age. | **S2** |
| `DEP-crate-budget` | Per-platform crate counts, per workspace member, against ceilings in `qa/deps-budget.json`. Ceilings RATCHET DOWN: an improvement lowers them and locks itself in; a regression is a red build that has to be argued for in the diff. Measured with `--filter-platform` for the two platforms that exist — the unfiltered resolve is 346 packages of which 62 are Windows-only and can never be acted on. | S2 |
| `DEP-no-new-duplicates` | A crate compiled at two versions is two compiles and two copies in the binary. The current 13–14 duplicates are pinned with the budget; a NEW one is red. Self-expiring in both directions: a duplicate that gets FIXED must be removed from the file, or the allowlist silently re-permits it when it returns. | S2 |
| `DEP-forbidden` | Dependencies that must not appear in a given binary at all (PM A10: "a laptop or wheeld build never compiles sqlx"). Current violations are recorded as `known_debt` with an owner so the gate names them without blocking; deleting the entry is what turns the ban on, and the gate fails if debt is recorded for something that is no longer true. | S2 |
| `DEP-binary-size` | Release binary size for what actually ships (engine, cli, host, api), ratcheting down, 2% tolerance for toolchain noise. Needs a real release build so it is `make size` and CI, never `make check`. The budget starts EMPTY and seeds from a real measurement — a ceiling nobody has measured is a number nobody can defend. | S2 |
| `POS-write-matches-refetch/*` | THE gate for the position ruling (PM, 710239f). Whatever the engine decides a position is, the response to the write and a later `/v1/board` refetch must say the same thing. Rounding and clamping are implemented twice — engine and client — and what reaches an operator is not "the maths is wrong" but the two halves drifting apart so a node moves on reload. | **S2** |
| `POS-move-write-matches-refetch` | The same agreement on the PATCH path, which is the one an operator actually exercises by dragging and is a different code path from create. | **S2** |
| `POS-move-clamps` | Dragging a node past the bound clamps to the i16 bounds on the PATCH path and returns the clamped value. | S2 |
...
| `WOW-token-not-on-disk` | After clone, the workspace holds no credential anywhere on disk: the literal token appears in no file, `.git/config` contains no `https://user:secret@host` remote, and `git remote -v` prints none. Found live on the production volume by the cloud adversary (PM, 15:55) — a real PAT in plaintext, readable by every uid that can reach the workspace and outliving the agent that used it. | **S1** |
| `WOW-token-not-on-disk-after-push` | The same property after a PUSH, which is where git is most tempted to persist a credential. A workspace clean after clone and dirty after push is a different bug from the one above and must not be able to hide behind it. | **S1** |
| `WOW-commit` | An agent edits a tracked file in its workspace and commits. Git identity is configured, or the commit fails with a legible error rather than silently doing nothing. | S2 |
| `WOW-push-needs-the-token` | CONTROL for the leg below. The same push is attempted first with the token removed from the environment and MUST fail. A push that succeeds on a cached helper, an ambient git config or a credential baked into the image proves nothing about the sandbox — the exact shape that produced three false greens on 2026-09-06. | control |
| `WOW-git-askpass-wired` | The engine's out-of-band git credential (`GIT_ASKPASS` → 0700 helper reading the token from the environment, SDK ddaa0e1) is set, executable by the child, and the token is in the child's environment. If the safe path does not work the agent reaches for the one that does — a token in the remote URL — so finding 036 returns as a consequence of this being broken. | **S1** |
| `WOW-commit-push` | The third leg of the operator's goal (clone → edit → push): an agent in the sandbox pushes to a throwaway branch `wow/<run-id>` on the real remote using its vault-held token, and the branch is deleted in the same run. Never main, never a role branch. Already happening in production (seven agent-opened PRs on 2026-09-06) — so this gate protects a live capability rather than proving a new one. | S2 |
...
| `ENG-journal-override-cannot-disable-recovery/{truncate,wal,delete}` | With `WHEEL_SQLITE_JOURNAL` forced to any mode sqlite knows, a database on the killing volume still ends up in a mode the filesystem can host. This is the gate the outage needed and did not have: the operator's own `TRUNCATE` collapsed both negotiation steps onto one target, the WAL-under-EXCLUSIVE branch was never reached, and the deploy died with "tried TRUNCATE, then TRUNCATE". An override on a recovery path must be provable, not plausible. | **S1** |
| `ENG-journal-override-invalid-fails-loudly` | A mode name sqlite has never heard of **may** stop the boot — that is a configuration error and failing fast beats silently picking something else — but the message must name `WHEEL_SQLITE_JOURNAL` and the offending value. Split out of the case above after a74e56d, because "must always boot" was the wrong rule for a typo. | S2 |
| `ENG-diskfull-says-so` | On a volume with no free space, the engine's FAILURE line names the disk being full. It does: `applying schema: database or disk is full`. The tmpfs must be created with the agent's uid/gid or the filler cannot be written, the volume stays EMPTY, and the engine's error is about an unwritable directory instead — a mistake I made and nearly reported as a defect. The failure line rather than literally line one, since a tracing banner legitimately precedes it and PM's requirement is about which CAUSE the operator is told. | S2 |
| `ENG-starts-with-unparseable-message` | **An engine still STARTS when a stored message cannot be parsed.** The parser must reject the ROW, not the boot. A row it cannot read takes the board down on every restart, so the system cannot recover by being turned off and on again — which is what a 502 that outlives a redeploy looks like. The row is written straight into sqlite with `CAST(x'FFFE003C2F41' AS TEXT)`, because the case is a row the engine did not create and would never accept: what a bad migration, a partial write or an older version leaves behind. | **S1** |
| `ENG-escaper-never-panics` | The envelope escaper never panics. Driven where it actually runs: `escape_envelope_body` has ONE production call site, `Message::envelope` at `supervisor/mod.rs:545`, which runs at **delivery** — so the agent must be STARTED and a delivered benign body must be found in the child's stdin transcript before any of this is evidence (`ENG-panic/escaper-runs`). Killing shape, measured against the pre-fix source: `<` or `</` then 9–10 ASCII bytes then a 3-byte character (8–10 for a 4-byte one), so the tag slice ends on a continuation byte. Walked over every offset 0..14 for em dash, emoji, CJK and a ZWJ family, plus PM's production line verbatim. | **S1** |
| `ENG-delivery-survives-escaper` | After those inputs the engine still **delivers**, not merely answers. Measured on the pre-fix engine at the moment of the panic: send dropped the connection, `/healthz` answered **200**, the container was **running**, and a benign message sent afterwards was never delivered. A tokio task unwinds alone, so the failure is a board that passes every healthcheck and does nothing. Liveness is not delivery, and only one of them was ever true. | **S1** |
| `ENG-starts-with-poison-ctx` / `ENG-injects-poison-ctx-without-panic` | A ctx node whose markdown is ordinary prose with an angle bracket and an em dash — the bytes PM pulled from the production `nodes` row — survives three restarts and a prompt composition. **Regression guard, NOT the reproduction:** both pass against the pre-fix engine too. ctx markdown reaches the child through `supervisor/prompt.rs`, which does no slicing and no escaping, so the same sentence is lethal as a message body and inert as ctx. | S3 |
| `ENG-escaper-engine-survives` | The engine is still serving **after** those inputs. A panic can take the process down after it has answered, so a 200 on the request is not proof the request was survivable. | **S1** |
| `HOST-reconcile-progress-advances` | During a reconcile of N>1 projects the 503 on project routes reports `to_restore == N` and a `restored` that **advances** between two observations, and the routes open when it completes. ADVERSARY 032 residual 2: a host working through fourteen sandboxes and a host wedged on the first answer the same 503, and that ambiguity made every healthcheck verdict during the outage unreadable. The counters exist (`wheel-host/src/lib.rs:547`); the only assertion on them today is `to_restore.is_some()` (`host_api.rs:1119`), and a counter that exists but never moves is exactly as ambiguous as no counter. Owner: API. | S2 |
| `INFRA-cargo-gate-honesty` | A cargo failure carrying no failing test, panic, compiler error or signal is reported **INCONCLUSIVE**, not red. Six worktrees share one `target-dir`; a bare `cargo` call outside `check.sh`'s lock rewrites artifacts underneath a run and produces `error: test failed` with no evidence at all. "Could not check" must not read as "failed" any more than it may read as "passed" — a merge gate that cries wolf is ignored exactly when it is right. | S2 |
| `INFRA-sandbox-facts` | `qa/tools/sandbox_facts.py` reports real `df`, real memory and the real toolchain paths **from inside a running sandbox**, never from a control plane. It prints free space *and* whether the directory is writable by the running uid, because "0% used" and "you can write here" are different facts — my own disk-full fixture was an unwritable root-owned mount reading as an empty one, and this tool shows it as `0% used  writable=NO  755 root:root` in a single line. Also doubles as an ADVERSARY 029 spot-check: `/opt/rust` must read `writable=NO`. | S3 |
| `INFRA-disk-budget-isolation` | **Owed when SDK lands A1 (per-project disk budget).** A project that exceeds its budget fails **alone** — every other project on the host is still serving. The failure mode worth gating is the one where a single tenant filling its quota takes the machine down with it, which is how a budget turns one project's bug into an outage. | **S1** |
| `ENG-second-connection-not-locked-out` | With the engine running on a NORMAL volume, a second connection to its database can **read and write**. Guards the property PM flagged: a transient `locking_mode=EXCLUSIVE` that is never given back trades a crash loop for an engine nothing else can open, and the query path opens this file a second time. Measured discriminator, not a formality — a held EXCLUSIVE blocks a second connection from both reads and writes. Runs today, unlike `ENG-concurrent-readers`, which BUG-022 leaves skipped until the engine can start on a hostile volume at all. | **S1** |
