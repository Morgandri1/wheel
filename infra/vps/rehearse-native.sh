...
#            serve, upgrade and migrate imply install -- they have nothing to measure without it.
#
# DO NOT EDIT THIS FILE WHILE A RUN IS IN FLIGHT. bash reads a script incrementally, so an edit
# mid-run makes the interpreter resume at a byte offset that now lands in the middle of a different
# line. That is not hypothetical: it happened during this lane's own rehearsal and produced
# "reason: command not found" plus a syntax error at the closing `fi`, after every check had
# already passed. Copy it, or wait.
...
# serve, upgrade and migrate all need a box install.sh has actually built, so asking for one of
# them implies the install phase. Without this, `--only serve` silently did nothing: the install
# phase never ran, `installed` stayed 0, and every check below it was skipped -- a rehearsal that
# exits without having rehearsed anything.
needs_install() {
    case "$1" in serve | upgrade | migrate) return 0 ;; *) return 1 ;; esac
}
run_phase() {
    [ -z "$only" ] && return 0
    [ "$only" = "$1" ] && return 0
    # The implied prerequisite, and only that one.
    [ "$1" = install ] && needs_install "$only" && return 0
    return 1
}
