...
mod recipients_tests {
    ...
    fn only_recipients_with_queued_work_are_named_and_each_once() {
        ...
        enqueue(&conn, MessageSender::User, waiting, "a".into(), None, None).unwrap();
        enqueue(&conn, MessageSender::User, waiting, "b".into(), None, None).unwrap();
        let m = enqueue(&conn, MessageSender::User, done, "c".into(), None, None).unwrap();
