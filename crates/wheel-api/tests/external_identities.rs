...
async fn app(external: Option<ExternalAuth>) -> (Router, Db) {
    app_with_origins(external, &[]).await
}

async fn app_with_origins(external: Option<ExternalAuth>, origins: &[String]) -> (Router, Db) {
    ...
    (wheel_api::build_router(state, origins), db)
}
...
async fn an_externally_provisioned_account_is_not_the_owner_account() {
    ...
    // And it cannot use the owner-only routes, which is what `owner` actually controls. The two
    // are refused for different reasons and the codes say which, rather than being flattened into
    // one assertion that would pass if either route started answering for the wrong reason:
    //
    //   * the identity routes exist in this mode and refuse a caller who is not the operator: 403.
    //   * `POST /v1/auth/users` is a LOCAL-auth route, so under any other mode it does not exist
    //     at all: 404. That is `require_local`, and it is the stronger of the two answers.
    for (method, uri, body, want) in [
        (
            "GET",
            "/v1/auth/external-identities",
            None,
        ...
        .await;
        assert_eq!(status, want, "an external account reached {uri}");
    }
}
...
/// Under `proxy_header` the credential is **ambient**: the proxy attaches it, so a hostile page
/// can make a victim's browser issue an authenticated request and the browser will comply — there
/// is no token for the page to fail to know. CORS does not cover it, because the engine proxy
/// accepts every verb and content type and is therefore not always preflighted.
///
/// So a request whose `Origin` is not on the allowlist is refused, and an EMPTY allowlist refuses
/// every origin rather than allowing every one. That empty case is the deployed posture
/// (`CORS_ALLOWED_ORIGINS` unset; the web app calls the API from its own server), which is exactly
/// why it must be the strict one.
#[tokio::test]
async fn a_cross_origin_page_may_not_spend_an_ambient_proxy_credential() {
    let (app, _db) = app(Some(proxy_external())).await;

    // No Origin: not something a page can cause, and the shape of every non-browser client.
    let (status, _) = call(
        &app,
        "GET",
        "/v1/auth/me",
    ...
    for origin in ["https://evil.example", "null", "http://localhost:3000"] {
        let (status, _) = call(
            ...
            None,
            None,
            &[("x-forwarded-user", "alice"), ("origin", origin)],
            true,
        )
        .await;
        assert_eq!(
            status,
            StatusCode::FORBIDDEN,
            "{origin} spent an ambient credential"
        );
    }
}

/// The deployer's own app is still allowed to be a browser app: an origin they listed passes, and
/// a near-miss of it does not.
#[tokio::test]
async fn an_allowed_origin_still_works_under_proxy_header_auth() {
    let allowed = "https://app.wheel.test";
    let (app, _db) = app_with_origins(Some(proxy_external()), &[allowed.to_string()]).await;
    ...
    let (status, _) = call(
        ...
        None,
        None,
        &[("x-forwarded-user", "alice"), ("origin", allowed)],
        true,
    )
    .await;
    assert_eq!(status, StatusCode::OK);

    for near_miss in ["https://app.wheel.test.evil", "http://app.wheel.test"] {
        let (status, _) = call(
            &app,
            "GET",
            "/v1/auth/me",
            None,
            None,
            &[("x-forwarded-user", "alice"), ("origin", near_miss)],
            true,
        )
        .await;
        assert_eq!(
            status,
            StatusCode::FORBIDDEN,
            "{near_miss} passed as {allowed}"
        );
    }
}

/// The CSRF refusal runs **before** the identity is resolved, so a cross-origin request never
/// reaches the database — and never provisions an account as a side effect of being refused.
#[tokio::test]
async fn a_refused_cross_origin_request_provisions_nobody() {
    let (app, db) = app(Some(proxy_external())).await;
    let before = wheel_api::auth::local::count_users(&db).await.unwrap();
    ...
    assert_eq!(status, StatusCode::FORBIDDEN);
    assert_eq!(
        wheel_api::auth::local::count_users(&db).await.unwrap(),
        before,
        "a refused request still created an account"
    );
}
