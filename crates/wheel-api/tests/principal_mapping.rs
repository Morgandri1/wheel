...
async fn linked_provisioning_refuses_an_unknown_subject_until_it_is_linked() {
    ...
    let account = wheel_api::auth::local::create_user(&db, "alice@example.com", "Correct-Horse-9!")
        .await
        .unwrap();
    external::link(&db, &cfg, &verified("alice", None), account.id)
        .await
        .expect("linked");
...
async fn a_subject_can_only_be_linked_once() {
    ...
    external::link(&db, &cfg, &verified("alice", None), a.id)
        .await
        .expect("first link");
    let second = external::link(&db, &cfg, &verified("alice", None), b.id).await;
