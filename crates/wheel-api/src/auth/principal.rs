...
//! about any of them. Today it is a local uuid or an identity provider's `sub`; either way it is
//! text that arrived from outside, so "whatever the token said" is not a safe answer to what it may
//! contain.
//!
