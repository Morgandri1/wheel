...
//! attacker reaches for first. Five properties, in the order they are enforced:
//!
//! 1. **Path confinement.** The suffix is refused if any segment could be read downstream as `..`
//!    or a separator, and is appended beneath this project's ingress base one segment at a time.
//!    Concatenating it let an anonymous caller climb from one project's ingress into another
//!    project's engine route on the host. The check runs before the lookup, so a refused path
//!    answers the same for every project id.
//! 2. **No existence oracle.** An unknown project is 404 and a project with ingress disabled is
...
//! 3. **Capability gate.** `capabilities.http` defaults to false and a malformed capabilities blob
//!    parses to false, so ingress is never enabled by accident.
//! 4. **Rate limit**, counted in Postgres so it holds across replicas.
//! 5. **Header scrubbing.** Every `x-wheel-*` header from the caller is dropped before we add our
...
use crate::http::hop;
use crate::routes::proxy::{refuse_ambiguous_path, upstream_url};
...
pub async fn ingress(
    ...
    let upstream = upstream_url(
        &state.ingress_base_url(&project_id),
        &rest,
        req.uri().query(),
    )?;
