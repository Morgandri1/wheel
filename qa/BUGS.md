...
### 041 — `wheel-on-wheel`'s own `GITHUB_TOKEN` cannot push, masking real signal on nearly every PR (S3, QA (mine), CLOSED, PR #18 89ceb67)

**Filed as 026, then 029, now 041** — collided first with `main`'s real 026
(Position-integer-cell), then with `main`'s real 029 (out-of-range position clamps silently on
migration) after this branch sat unmerged through several more rounds of renumbering on main.
Content unchanged from filing.
