/**
 * Does the browser's auth mode agree with the API's?
 *
 * The client learns its mode from a build-time env var and the server from a runtime one, set in
 * different dashboards by different people. When they disagree nobody can log in, and no test
...
export function authModeMismatch(
  ...
  if (client === "mock") {
    return `The browser is in mock auth mode but a real API answered, and it reports ${server}. Mock sends a fixed fake token that a real API will reject. NEXT_PUBLIC_AUTH_MODE is probably unset — it defaults to mock.`;
  }
  const expected = EXPECTED[client];
  if (expected === server) return null;
  return `The browser is built for ${client} auth, which needs an API in ${expected} mode, but the API reports ${server}. Nobody will be able to log in. Fix whichever is wrong: NEXT_PUBLIC_AUTH_MODE on the web deployment, or AUTH_MODE on the API.`;
}
